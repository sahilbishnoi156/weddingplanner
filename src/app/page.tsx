...
export const metadata: Metadata = {
  title: "Wedding Manager: Effortless Guest Management & Instant Wedding Codes",
  description:
  ...
  openGraph: {
    title: "Wedding Manager: Instant Guest List Tool",
    description:
    ...
    images: [
      {
        ...
        width: 1200,
        height: 630,
        alt: "Wedding Manager dashboard preview",
      },
    ],
  },
  twitter: {
    card: "summary_large_image",
    title: "Wedding Manager: Guest Management",
    description:
...
export default function Landing() {
  return (
    <main className="min-h-screen  text-white font-sans tracking-tight dark:bg-foreground">
      <div className="max-w-4xl mx-auto p-4 flex flex-col gap-10">
        <header className="text-center py-4 border-b border-neutral-700">
          <h1 className="text-4xl sm:text-5xl md:text-6xl font-extrabold leading-tight mb-2 font-display dark:text-white text-black">
            Wedding Manager
          </h1>
          <p className="text-xl md:text-2xl font-medium dark:text-neutral-200 text-black max-w-2xl mx-auto">
            Fastest way to organize your guest list and manage your wedding in one place.
          </p>
        </header>

        <section className="grid md:grid-cols-2 gap-10 items-stretch dark:text-white text-black">
          {/* Left info + Join form */}
          <div>
            ...
            <div className="mt-8">
              <h3 className="text-lg font-semibold dark:text-neutral-100 text-neutral-600 mb-1">Privacy & Security</h3>
              <ul className="dark:text-neutral-400 text-neutral-600 text-sm space-y-1 list-disc list-inside">
        ...
        <footer className="mt-4 pt-4 border-t border-neutral-700 text-center text-xs text-neutral-500">
          &copy; {new Date().getFullYear()} Wedding Manager. All rights reserved. | No account required | <span role="img" aria-label="wedding rings">💍</span>
        </footer>
      </div>
    </main>
  );
