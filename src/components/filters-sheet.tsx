...
export function FiltersSheet() {
  const { cities, categories, draft, applied, setDraft, applyFilters, clearFilters } = useStore.getState();

  const [open, setOpen] = useState(false)

  const appliedCount = useMemo(() => {
    const cityCount = applied.cityIds.length
    const catCount = Object.values(applied.categories).filter((v) => v !== "any").length
                  ...
                  <label key={c.id} className="flex items-center gap-2">
                    <Checkbox
                      ...
                      onCheckedChange={(v) => {
                        ...
                        setDraft({
                          ...draft,
                          cityIds: on ? [...draft.cityIds, c.id] : draft.cityIds.filter((id) => id !== c.id),
                        })
                      }}
                    />
                ...
                return (
                  <div key={cat.id} className="flex items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2">
                      {(["any", "checked", "unchecked"] as const).map((opt) => (
                        <Button
                          key={opt}
                          size="sm"
                          ...
                          onClick={() =>
                            setDraft({
                              ...draft,
                              categories: { ...draft.categories, [cat.id]: opt },
                            })
                          }
                        >
                          {opt}
